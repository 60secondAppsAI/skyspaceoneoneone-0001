        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPrograms = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPrograms">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Program</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ProgramId" type="text" placeholder="Enter ProgramId" v-model="programToAdd.programId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="programToAdd.name"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="programs" :row-key="record => record.ProgramId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ProgramPictureView :programs="programs" />
          </div>
...
import ProgramPictureView from './ProgramPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'programId',
          visible: true,
          scopedSlots: { customRender: 'programId' },
          sorter: true
          //sorter: (a, b) => a.programId - b.programId,
          //sorter: (a, b) => a.programId.localeCompare(b.programId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      sortBy: 'programId',           // Column to sort by
  ...
  methods: {
    ...
    async renderProgramsTable() {
      ...
      let programsTableData = [];
      for (let i = 0; i < this.programs.length; i++) {
        programsTableData.push({
          id: i,
          programId: this.programs[i].programId,
          year: this.programs[i].year,
          date: this.programs[i].date,
          competitionId: this.programs[i].competitionId,
          programId: this.programs[i].programId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-programs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForProgramsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalPrograms = false;

      const currentDate = new Date().getTime();
      this.programToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.programToAdd);
      console.log(jsonData);
      
      const res = await ProgramService.addProgram(jsonData);
