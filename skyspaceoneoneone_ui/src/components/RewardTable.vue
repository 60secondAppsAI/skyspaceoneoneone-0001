        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalRewards = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRewards">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Reward</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RewardId" type="text" placeholder="Enter RewardId" v-model="rewardToAdd.rewardId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="rewardToAdd.description"></base-input>
  <base-input label="PointsRequired" type="text" placeholder="Enter PointsRequired" v-model="rewardToAdd.pointsRequired"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="rewards" :row-key="record => record.RewardId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <RewardPictureView :rewards="rewards" />
          </div>
...
import RewardPictureView from './RewardPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'rewardId',
          visible: true,
          scopedSlots: { customRender: 'rewardId' },
          sorter: true
          //sorter: (a, b) => a.rewardId - b.rewardId,
          //sorter: (a, b) => a.rewardId.localeCompare(b.rewardId),
        },
        {
          title: 'Description',
		...
		dataIndex: 'pointsRequired',
          visible: true,
          scopedSlots: { customRender: 'pointsRequired' },
          sorter: true
          //sorter: (a, b) => a.pointsRequired - b.pointsRequired,
          //sorter: (a, b) => a.pointsRequired.localeCompare(b.pointsRequired),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'rewardId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRewardsTable() {
      ...
      let rewardsTableData = [];
      for (let i = 0; i < this.rewards.length; i++) {
        rewardsTableData.push({
          id: i,
          rewardId: this.rewards[i].rewardId,
          year: this.rewards[i].year,
          date: this.rewards[i].date,
          competitionId: this.rewards[i].competitionId,
          rewardId: this.rewards[i].rewardId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-rewards',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRewardsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalRewards = false;

      const currentDate = new Date().getTime();
      this.rewardToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.rewardToAdd);
      console.log(jsonData);
      
      const res = await RewardService.addReward(jsonData);
