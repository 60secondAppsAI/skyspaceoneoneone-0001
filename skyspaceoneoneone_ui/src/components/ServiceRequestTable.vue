        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalServiceRequests = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalServiceRequests">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add ServiceRequest</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ServiceRequestId" type="text" placeholder="Enter ServiceRequestId" v-model="serviceRequestToAdd.serviceRequestId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="serviceRequestToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="serviceRequests" :row-key="record => record.ServiceRequestId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ServiceRequestPictureView :serviceRequests="serviceRequests" />
          </div>
...
import ServiceRequestPictureView from './ServiceRequestPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    ServiceRequestPictureView
  },

  data() {
    return {
      modalServiceRequests: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'serviceRequestId',
          visible: true,
          scopedSlots: { customRender: 'serviceRequestId' },
          sorter: true
          //sorter: (a, b) => a.serviceRequestId - b.serviceRequestId,
          //sorter: (a, b) => a.serviceRequestId.localeCompare(b.serviceRequestId),
        },
        {
          title: 'Description',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} serviceRequests`,
      },
      ...
      sortBy: 'serviceRequestId',           // Column to sort by
  ...
  methods: {
    ...
    async renderServiceRequestsTable() {
      ...
      let serviceRequestsTableData = [];
      for (let i = 0; i < this.serviceRequests.length; i++) {
        serviceRequestsTableData.push({
          id: i,
          serviceRequestId: this.serviceRequests[i].serviceRequestId,
          year: this.serviceRequests[i].year,
          date: this.serviceRequests[i].date,
          competitionId: this.serviceRequests[i].competitionId,
          serviceRequestId: this.serviceRequests[i].serviceRequestId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-service-requests',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForServiceRequestsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalServiceRequests = false;

      const currentDate = new Date().getTime();
      this.serviceRequestToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.serviceRequestToAdd);
      console.log(jsonData);
      
      const res = await ServiceRequestService.addServiceRequest(jsonData);
